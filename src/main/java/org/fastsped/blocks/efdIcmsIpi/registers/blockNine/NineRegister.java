...
public class NineRegister implements RegisterFactory {
    ...
    private int compareFinallyBlocks(String s1, String s2, char charBlockFinally) {
        String blockFinally = String.valueOf(charBlockFinally);
        if (s1.startsWith(blockFinally) && s2.startsWith(blockFinally))
            return s1.compareTo(s2);
        else if (s1.startsWith(blockFinally))
            return 1;
        else if (s2.startsWith(blockFinally))
            return -1;
